...
export default function AdminDashboard() {
  ...
  const totalTLsMes = tls.reduce((s, t) => s + (t.total_valorizado || 0), 0);
  const totalMes = totalOMsMes + totalTLsMes;
    ...
    <div className="page-content fade-in">
      ...
      <div className="stats-grid" style={{ marginBottom:28 }}>
        <div className="stat-card">
          ...
          <div className="stat-value money" style={{ fontSize:22 }}>{formatMoney(totalMes)}</div>
          <div className="stat-label">Valorizado este mes (OMs + TLs)</div>
        </div>
        <div className="stat-card">
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
          <h2 className="section-title" style={{ marginBottom:0 }}>
            ...
            Órdenes de Mantenimiento Recientes
          </h2>
      ...
      {/* Trabajos Libres recientes */}
      {tls.length > 0 && (
        <div>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
            <h2 className="section-title" style={{ marginBottom:0 }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" strokeWidth="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/></svg>
              Trabajos Libres (Sin OM)
              <span style={{ fontSize:12, fontWeight:500, color:'#64748b', marginLeft:8 }}>{tls.length} registrado(s)</span>
            </h2>
          </div>
          <div className="card-gradient" style={{ padding:0, overflow:'hidden' }}>
            <table className="data-table">
              <thead>
                <tr><th>Descripción</th><th>Cuadrilla</th><th>Fecha</th><th>Total</th><th>Estado</th><th>Detalle</th></tr>
              </thead>
              <tbody>
                {tls.slice(0, 10).map(tl => (
                  <tr key={tl.id}>
                    <td style={{ fontSize:13, color:'#94a3b8', maxWidth:200 }}>
                      <div style={{ overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{tl.descripcion}</div>
                    </td>
                    <td style={{ fontSize:12, color:'#64748b' }}>{tl.cuadrilla_nombre || '—'}</td>
                    <td style={{ fontSize:12, color:'#64748b' }}>{tl.fecha}</td>
                    <td className="money">{formatMoney(tl.total_valorizado || 0)}</td>
                    <td>
                      <span className={`badge ${
                        tl.estado === 'pendiente' ? 'badge-amber'
                        : tl.estado === 'afiliado' ? 'badge-green'
                        : 'badge-red'
                      }`}>
                        {tl.estado === 'pendiente' ? '🟡 Pendiente'
                          : tl.estado === 'afiliado' ? `✓ OM ${tl.numero_om_afiliada}`
                          : '⚠ Sin cubrir'}
                      </span>
                    </td>
                    <td>
                      <Link
                        href={`/supervisor/trabajos-libres/${tl.id}`}
